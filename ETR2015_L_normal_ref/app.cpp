...
static void tail_reset(void); // 尻尾モータの遊びをなくす
static void start_ready(void); // スタート待機

// 大域宣言

// pid制御関連の大域宣言
static int32_t diff[2];    // 目標値と測定値の差分の累積値
static int32_t integral;   // 偏差の積分値
static float kp = 0.36F;   // Pのゲイン値
static float ki = 1.2F;    // Iのゲイン値
static float kd = 0.027F;  // Dのゲイン値

// 光センサから得た値
static int32_t light_black;   // 黒の反射値
static int32_t light_white;   // 白の反射値
static int32_t light_gray;    // 灰の反射値
...
void main_task(intptr_t unused)
{
    ...
    int8_t forward;          // 前後進値
    int8_t turn;             // 旋回値
    int8_t p, i, d;          // pid制御の操作量
    int8_t pwm_L, pwm_R;     // 左右モータの出力値
...
// 引数 :
// 返り値 :
// 概要 : 巻き込んでしっぽモータの遊びをなくし、しっぽモータのエンコーダ値をリセット
//*****************************************************************************
...
static void start_ready(void)
{
    ...
    while(1)
    {
        ...
        ev3_speaker_play_tone(700, 60);
        // 完全停止用角度に手動で調節
        //tail_control(TAIL_ANGLE_STAND_UP + tail_butt);
        tail_control(90);
